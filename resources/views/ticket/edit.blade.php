...
<div class="container-fluid pt-3">
    ...
    <form enctype="multipart/form-data" class="mt-5 entry-form" action="{{ route('ticket.edit.put', [$project, $ticket]) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="ticket_name" class="form-control" value="@if (old('ticket_name')){{old('ticket_name') }}@else{{ $ticket->ticket_name }}@endif">
        </div>
        ...
        <div class="date-wrapper">
            <div class="start-date-wrapper mb-4">
                ...
                <input type="date" name="start_date" class="form-control" value="{{ empty(@old('start_date')) ? $start_date_f : @old('start_date') }}">
            </div>
            <div class="end-date-wrapper mb-4">
                <label for="end_date" class="form-label">期日</label>
                <input type="date" name="end_date" class="form-control" value="{{ empty(@old('end_date')) ? $end_date_f : @old('end_date') }}">
            </div>
        </div>

        <div class="mb-4">
            <label for="content" class="form-label">内容</label>
            <textarea class="form-control" name="content" id="content" cols="30" rows="10">{{ empty(old('content')) ? $ticket->content : old('content') }}</textarea>
        </div>
        <div class="mb-4 person-wrapper">
